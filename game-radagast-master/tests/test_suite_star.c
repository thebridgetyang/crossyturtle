...
void test_vec_distance() {
  assert(vec_distance(VEC_ZERO, VEC_ZERO) == 0);
  assert(vec_distance(VEC_ZERO, (vector_t){5, 0}) == 5);
  assert(vec_distance((vector_t){0, 0}, (vector_t){5, 2}) ==
         sqrt(pow(5, 2) + pow(2, 2)));
}
